...
for i = 1:numel(contents)
    ...
    for j = 1:numel(objects)
        ...
        if isfield(obj.metadata,'volumeRealUnitsHalfExtraPts')
           continue;
        end
        ...
        %Randomly choose (approximately) half of the GAPDH points.
        sel = rand(size(x));
        tmp=find(sel<.5);
        ...
        %Use extra points to fill up the nucleus too.
        mask = obj.object_mask.mask;
        dapiMask = obj.channels.dapi.processor.mask;
        extraFactor = numel(find(dapiMask))/numel(find(mask));
        numPtsToUse = round(numel(x)*(1+extraFactor));
        ...
        [vOrig cellTopOrig cellBottomOrig] = findVol_Expand2(obj, color, x, y, z, 1);
        ...
        % xf, yf, zf.
        [xf yf zf] = fillVol2(obj, color, cellTopOrig, cellBottomOrig, numPtsToUse, 1);
        ...
        [vFake cellTopFake cellBottomFake] = findVol_Expand2(obj, color, xf, yf, zf, 1);
        ...
        while 'true'
            ...
            [xf yf zf] = fillVol2(obj, color, cellTopOrig, cellBottomOrig, numPtsToUse, expFactor);
            
            [vFake cellTopFake cellBottomFake] = findVol_Expand2(obj, color, xf, yf, zf, expFactor);
        ...
        objects(j).metadata.volumeRealUnitsHalfExtraPts = volumeRealUnits;
    end
    fprintf('Saving %s\n',contents(i).name);
