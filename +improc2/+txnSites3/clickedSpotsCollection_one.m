classdef clickedSpotsCollection_one < ...
...
%         [dependencyClassNames{1:length(channels)}] = deal('improc2.interfaces.FittedSpotsContainer');
    ...
    methods
        %Create Object
        function p = clickedSpotsCollection_one(objectHandle, varargin)
            ip = inputParser;
...
%         
        ...
        function deleteLastClickedSpot(p)            
            data = p.objectHandle.getData(p.dataNodeLabel);
                if length(data.ClickedXs) < 2
                    ...
                    data.Z = data.Z(1:(end-1));
                    data.Intensity = data.Intensity(1:(end-1));
                    data.parentNodeName = data.ParentNodeName(1:(end-1));
                    data.pointID = data.pointID(1:(end-1));
                end
        ...
        function deleteClickedSpot(p,x)
            ...
            for k = 1:length(rnaChannels)
                ...
                objectHandle, rnaChannelSwitch, 'clickedSpotsCollection_one');
