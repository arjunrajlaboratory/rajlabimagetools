classdef clickedSpotsCollection < ...
        interfaces.manualSpotCollection & improc2.interfaces.NodeData
    ...
    properties (Constant = true)
        dependencyClassNames = {'improc2.interfaces.FittedSpotsContainer', 'improc2.interfaces.FittedSpotsContainer', 'improc2.interfaces.FittedSpotsContainer', 'improc2.interfaces.FittedSpotsContainer', 'improc2.interfaces.FittedSpotsContainer'};
...
%         [dependencyClassNames{1:length(channels)}] = deal('improc2.interfaces.FittedSpotsContainer');
    ...
    methods
        %Create Object
        function p = clickedSpotsCollection(objectHandle, varargin)
            ip = inputParser;
            ip.addOptional('nodeName', 'ManuallyClickedSpots', @ischar);
            ip.parse(varargin{:});
        ...
        function addClickedSpot(p, x, y)
            ...
            data.pointID = [data.pointID; repmat(length(data.ClickedXs), length(p.parentNodeLabels), 1)];
...
%         
        ...
        function deleteClickedSpot(p,x)
            ...
            for k = 1:length(rnaChannels)
                ...
                objectHandle, rnaChannelSwitch, 'clickedSpotsCollection');
