function out = FourColorSpotInspector(objectHandle,snpNode,backgroundNode) %,guideChannel,snpChannel1,snpChannel2)
...
resultsBackground = objectHandle.getData(backgroundNode);
backgroundChannel = resultsBackground.idMap.channels{2}; % by convention, the second channel listed is background.
backgroundName = resultsBackground.idMap.names{2};
backgroundRefChannel = resultsBackground.idMap.channels{1}; % reference channel.
backgroundRefName = resultsBackground.idMap.names{1};
...
img_gfp = readmm(objectHandle.getImageFileName(backgroundChannel));
...
gf1stack = rectcropmulti(img_gfp.imagedata,R);
...
% Now deal with background spots

backgroundSpots = resultsBackground.data.(backgroundChannel).position;
refBackgroundIndex = resultsBackground.data.(backgroundRefChannel).chan2_ID > 0;
backgroundRefIndex = resultsBackground.data.(backgroundRefChannel).chan2_ID(refBackgroundIndex);
...
alldata.backgroundName = backgroundName;
alldata.backgroundChannel = backgroundChannel;
alldata.backgroundSpots = backgroundSpots;
alldata.refBackgroundIndex = refBackgroundIndex;
alldata.backgroundRefIndex = backgroundRefIndex;
...
backgroundSpots = alldata.backgroundSpots;
refBackgroundIndex = alldata.refBackgroundIndex;
backgroundRefIndex = alldata.backgroundRefIndex;
...
showSpotLocations(backgroundSpots(backgroundRefIndex,:), currentZ,'go');
hold off
...
if strcmp(alldata.bg.SelectedObject.String,'Scale individual planes')
    imshow(gf1stack(:,:,currentZ),[]);
else
    imshow(gf1stack(:,:,currentZ),[min(gf1stack(:)) max(gf1stack(:))]);
end
...
hold on
showSpotLocations(backgroundSpots,currentZ,'go')
showSpotLocations(guideSpots(refBackgroundIndex,:),currentZ,'wo')
hold off
title([alldata.backgroundChannel, ' label: ', alldata.backgroundName]);
ax4 = gca;
