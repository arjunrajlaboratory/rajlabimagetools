...
function Hs = shiftTiles(Hs)
    ...
    Td.currImageOrig = imread(Td.filePaths{Td.currRow,Td.currCol,Td.chanIndex});
    Td.rightImageOrig = imread(Td.filePaths{Td.currRow,Td.currCol+1,Td.chanIndex});
    Td.downImageOrig = imread(Td.filePaths{Td.currRow+1,Td.currCol,Td.chanIndex});
    Td.downRightImageOrig = imread(Td.filePaths{Td.currRow+1,Td.currCol+1,Td.chanIndex});
    ...
    for r = minR:sizeR:(minR + (sizeR * (numRows - 1)))
        ...
        for c = minC:sizeC:(minC + (sizeC * (numCols - 1)))
            ...
            for index = 1:numel(Td.foundChannels)
                canvases(1,index) = {zeros(sizeR,sizeC)};
            end
